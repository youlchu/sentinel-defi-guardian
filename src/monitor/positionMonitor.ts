...
export class PositionMonitor {
  ...
  private attemptReconnect(): void {
    ...
    const jitterMin = this.jitterFactor;
    const jitterMax = 1 + (1 - this.jitterFactor);
    const jitter = jitterMin + Math.random() * (jitterMax - jitterMin);
    
    const delay = Math.floor(cappedDelay * jitter);

    console.log(
      `[WEBSOCKET] Attempting to reconnect in ${delay}ms (${this.reconnectAttempts}/${this.maxReconnectAttempts})`
    );
  ...
  private subscribeToAddress(address: string): void {
    ...
    console.log(
      `[WEBSOCKET] Subscribed to account: ${address} (ID: ${subscriptionId})`
    );
  }
  ...
  private unsubscribeFromAddress(address: string): void {
    ...
    console.log(
      `[WEBSOCKET] Unsubscribed from account: ${address} (ID: ${subscriptionId})`
    );
  }
    ...
    if (message.method === "accountNotification") {
      ...
      if (notification && notification.result && notification.result.value) {
        const pubkey = notification.result.context?.slot
          ? this.findAddressBySubscriptionId(notification.subscription)
          : null;
      ...
      console.log(
        `[WEBSOCKET] Subscription confirmed with ID: ${message.result}`
      );
  ...
  private async processAccountUpdate(address: string): Promise<void> {
    try {
      ...
      const addressPositions = currentPositions.filter((p) =>
        p.owner.equals(pubkey)
      );
      const previousAddressPositions = Array.from(
        this.previousPositions.values()
      ...
      console.error(
        `[WEBSOCKET] Error processing account update for ${address}:`,
        error
      );
    }
  }
  ...
  private hasPositionChanged(previous: Position, current: Position): boolean {
    if (Math.abs(previous.healthFactor - current.healthFactor) > 0.001)
      return true;
    ...
    for (let i = 0; i < previous.collateral.length; i++) {
      ...
      if (
        Math.abs(prevColl.amount - currColl.amount) > 0.001 ||
    ...
    for (let i = 0; i < previous.debt.length; i++) {
      ...
      if (
        Math.abs(prevDebt.amount - currDebt.amount) > 0.001 ||
  ...
  private notifyPositionChange(change: PositionChange): void {
    console.log(
      `[MONITOR] Position ${change.changeType}: ${change.position.id} (Protocol: ${change.position.protocol})`
    );

    if (change.changeType === "updated" && change.previousPosition) {
      console.log(
        `[MONITOR] Health factor changed: ${change.previousPosition.healthFactor} -> ${change.position.healthFactor}`
      );
    }
  ...
  async fetchPositionsForAddress(owner: PublicKey): Promise<Position[]> {
    ...
    try {
      const [marginfiPositions, kaminoPositions, driftPositions] =
      ...
      if (marginfiPositions.status === "fulfilled") {
        ...
        console.error(
          "[MARGINFI] Error fetching positions:",
          marginfiPositions.reason
        );
      }
      ...
      if (kaminoPositions.status === "fulfilled") {
        ...
        console.error(
          "[KAMINO] Error fetching positions:",
          kaminoPositions.reason
        );
      }
      ...
      if (driftPositions.status === "fulfilled") {
        ...
        console.error(
          "[DRIFT] Error fetching positions:",
          driftPositions.reason
        );
      }
    } catch (error) {
      console.error(
        `[MONITOR] Error fetching positions for ${owner.toBase58()}:`,
        error
      );
    }
  ...
  async fetchAllPositions(): Promise<Position[]> {
    ...
    for (const address of this.watchedAddresses) {
      fetchPromises.push(
        (async () => {
          try {
            const pubkey = new PublicKey(address);
            const addressPositions = await this.fetchPositionsForAddress(
              pubkey
            );
            ...
            console.error(
              `[MONITOR] Error fetching positions for ${address}:`,
              error
            );
          }
        })()
    ...
    console.log(
      `[MONITOR] Fetched ${positions.length} total positions across all addresses`
    );
  ...
  private async fetchKaminoPositions(owner: PublicKey): Promise<Position[]> {
    ...
    try {
      const KAMINO_PROGRAM_ID = new PublicKey(
        "KLend2g3cP87ber41aPn9Q5kkdCZNxMWTKZLGvBKgvV"
      );
